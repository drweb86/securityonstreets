...
namespace HDE.IpCamClientServer.Server.Core.ImageProcessingHandlers
{
    ...
    public static class GrayScaleImageHelper
    {
        ...
        public static void GetMultiplier1(
            int width,
            int height,
            int stride,
            byte[] data1,
            Bitmap bitmap2,
            out double[,] widthHeightDataIntensity)
        {
            if (bitmap2.PixelFormat != PixelFormat.Format8bppIndexed)
            {
                throw new NotSupportedException("Pixel format is not supported (bitmap2).");
            }

            var bounds = new Rectangle(0, 0, width, height);
            widthHeightDataIntensity = new double[width, height];

            BitmapData bitmapData2 = bitmap2.LockBits(bounds, ImageLockMode.ReadOnly, bitmap2.PixelFormat);
            var data2 = new byte[bitmap2.Height * bitmapData2.Stride];
            Marshal.Copy(bitmapData2.Scan0, data2, 0, bitmap2.Height * bitmapData2.Stride);
            ...
            for (int widthI = 1; widthI < (width - 1); widthI++)
            {
                for (int heightI = 1; heightI < (height - 1); heightI++)
                {
                    ...
                    var b11 = data2[ToDataPosition(     widthI - 1, heightI - 1,    bitmapData2.Stride)];
                    var b12 = data2[ToDataPosition(     widthI - 1, heightI,        bitmapData2.Stride)]; ;
                    var b13 = data2[ToDataPosition(     widthI - 1, heightI + 1,    bitmapData2.Stride)];
                    var b21 = data2[ToDataPosition(     widthI,     heightI - 1,    bitmapData2.Stride)];
                    var b22 = data2[ToDataPosition(     widthI,     heightI,        bitmapData2.Stride)];
                    var b23 = data2[ToDataPosition(     widthI,     heightI + 1,    bitmapData2.Stride)];
                    var b31 = data2[ToDataPosition(     widthI + 1, heightI - 1,    bitmapData2.Stride)];
                    var b32 = data2[ToDataPosition(     widthI + 1, heightI,        bitmapData2.Stride)];
                    var b33 = data2[ToDataPosition(     widthI + 1, heightI + 1,    bitmapData2.Stride)];
                    ...
                    widthHeightDataIntensity[widthI, heightI] = Math.Round((c11 + c12 + c13 + c21 + c22 + c23 + c31 + c32 + c33) / 9.0);
                }
            }
